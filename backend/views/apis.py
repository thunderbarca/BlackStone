                  ...
                  'menuInfo': [
                      {'title': '常规管理', 'icon': 'fa fa-address-book', 'href': '', 'target': '_self', 'child': [
                          ...
                          {'title': '容器管理', 'href': reverse("back_container_show"), 'icon': 'fa fa-file-text',
                           'target': '_self'},
                          {'title': '成绩管理', 'href': reverse("back_resolver_show"), 'icon': 'fa fa-lemon-o',
                           'target': '_self'},
...
class PlayersApiView(AdminRequiredMixin, View):
    ...
    def get(request):
        ...
        if len(search_dict) == 0:
            ...
            user_list = Players.objects.filter(**search_dict).order_by('create_date')
